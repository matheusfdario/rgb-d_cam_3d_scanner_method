    ...
    while True:
        ...
        if(get_depth):
            ...
            if(frame_number==0):
                ...
                # brute force correction of deprojection error
                P0X = brute_force_pointcloud_correction(P0,data0)
                P0 = P0X
                dist = distance.cdist(P0, data0)
                print("min", dist.min())
            else:
                if frame_number in sel_frames:
                    ...
                    for i, pix_new in enumerate(points1):
                        ...
                        while(deprojection_check):
                            try:
                                ...
                                print(i, c, d, 'OUT1')
                    # brute force correction of deprojection error
                    P1X = brute_force_pointcloud_correction(P1, data1)
                    P1 = P1X
                    dist = distance.cdist(P1, data1)
                    print("min", dist.min())
                    combined_frames += 1
                    if(combined_frames<len(matches_list)):
                        ...
                        for i, pix_new in enumerate(points0_next):
                            ...
                            #     deprojection_check = False
                                ...
                                print(i, c, d, 'OUT1')
                        # brute force correction of deprojection error
                        P0_next_X = brute_force_pointcloud_correction(P0_next, data1)
                        P0_next = P0_next_X
                        dist = distance.cdist(P0, data1)
                        print("min", dist.min())
                    P0,P1 = filter_invalid_3D_points(P0,P1,0.05)
                    P0_list.append(P0)
                    P1_list.append(P1)
                    A = np.mat(P1)
                    B = np.mat(P0)
                    ...
                    P1_T = A_transformed
                    P1_T_list.append(P1_T)
                    A = np.mat(data1)
                    B = np.mat(data0)
                    n = A.shape[0]
                    centroid_A = np.mean(A, axis=0)
                    centroid_B = np.mean(B, axis=0)
                    tc = -Rc * centroid_A.T + centroid_B.T      #test to use transform with a diferent pointset
                    A_transformed = (Rc * A.T) + np.tile(tc, (1, n))
                    A_transformed = A_transformed.T
                    data_end_T = np.asarray(A_transformed)
                    #data_str_T = data_str
                    pointclouds.append(data1)
                    pointclouds_T.append(data_end_T)
...
xm = data_merged[::100, 0]
ym = data_merged[::100, 1]
zm = data_merged[::100, 2]
